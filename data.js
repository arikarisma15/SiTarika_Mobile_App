export const CategoryList = [
  {
    id: 1,
    categoryName: 'Terkini',
  },
  {
    id: 2,
    categoryName: 'Tari Tradisional',
  },
  {
    id: 3,
    categoryName: 'Tari Moderen',
  },
  {
    id: 4,
    categoryName: 'Berita Tari Luar Negeri',
  },

];
export const newsData = [
  {
    title: 'Joe Biden Tersenyum Turun dari Pesawat Disambut Tari Pendet',
    description: 'Pada Instagram Live G20 Indonesia, momen penuh kebahagiaan terjadi ketika Joe Biden tiba di Bali, sebuah pulau eksotis yang kaya akan budaya dan keindahan alam. Kehadiran sang Presiden Amerika Serikat ini disambut dengan meriah oleh tarian khas Bali yang memukau, menciptakan suasana kehangatan dan kegembiraan di sekitarnya.Para penonton yang menyaksikan secara langsung melalui platform Instagram Live dapat merasakan getaran keceriaan dari layar mereka saat Joe Biden menuruni tangga pesawat. Sorotan kamera mengabadikan detik-detik ketika Presiden dengan ramah tersenyum, memberikan kesan hangat dan ramah kepada seluruh penonton di seluruh dunia Suasana Bali yang kental dengan keunikan dan tradisi seni tampil dalam keindahan tarian yang mempesona, seolah memberikan sambutan khusus untuk tamu istimewa ini. Joe Biden, di tengah gemerlap lampu dan nuansa tradisional Bali, melangkah dengan tenang menuju mobil yang menunggu, sementara senyumannya mencerminkan apresiasi terhadap keindahan dan keramahan yang dihadiahkan oleh pulau ini.',
    imageUrl: 'https://i.pinimg.com/originals/42/73/11/427311dc24f2297f8d8df36cc5c6cf95.jpg',
    category: 'Tarian',
    uploadBy: 'Admin SiTarika',
    createdAt: '16 November 2023'
  },
  {
    title: 'Wisatawan Kepincut Sendratari Meras Gandrung Banyuwangi',
    description: 'Ini pertama kalinya saya berkunjung ke Banyuwangi. Sangat menarik pentas seni ini, bercerita tentang bagaimana menjadi seorang Gandrung. Dan lokasi pertunjukannya juga sangat keren," kata Ari saat melihat Meras Gandrung pada Rabu (22/3/2023)',
    imageUrl: 'https://i.pinimg.com/originals/92/bd/b3/92bdb36d69ac2d460a7a8b1a5b392c9f.jpg',
    category: 'Tarian',
    uploadBy: 'Admin SiTarika',
   createdAt: '16 November 2023'
  },
  // {
  //   title: 'Tarian Jawa Tengah Bikin Kagum',
  //   description: 'Tari Gambyong adalah salah satu tarian tradisional Jawa yang kaya akan sejarah dan budaya Indonesia. Tarian ini sering kali dipentaskan dalam berbagai upacara adat, seperti pernikahan, upacara adat, atau acara budaya, dan memiliki makna mendalam di balik gerakan-gerakan yang dilakukan.Tarian ini sudah ada sejak zaman dahulu dan mulai ditampilkan di lingkungan Istana Mangkunegaran pada era 1916-1944 yang sangat terkenal padan jaman itu. (Senin, 22 Agu 2022 16:54 ~ detikJateng)',
  //   imageUrl: 'https://i.pinimg.com/564x/c7/a4/f4/c7a4f4c6451828baa66311f7f1ea4487.jpg',
  //   category: 'Tarian',
  //   uploadBy: 'Admin SiTarika',
  //   createdAt: '16 November 2023'
  // },
  
];
